...
public class AttributeHidingFactorProjectCalculator extends ProjectCalculator
{
	...
	private class Visitor extends JavaRecursiveElementVisitor
	{
		...
		public void visitField(PsiField field)
		{
			...
			if(field.hasModifierProperty(PsiModifier.PRIVATE) || containingClass.hasModifierProperty(PsiModifier.PRIVATE))
			{
				//dodn't do anythng
			}
			else if(field.hasModifierProperty(PsiModifier.PROTECTED) || containingClass.hasModifierProperty(PsiModifier.PROTECTED))
			{
				totalVisibility += getSubclassCount(containingClass);
			}
			else if((field.hasModifierProperty(PsiModifier.PUBLIC) || containingClass.isInterface()) && containingClass.hasModifierProperty(PsiModifier
					.PUBLIC))
			{
				numPublicAttributes++;
			}
