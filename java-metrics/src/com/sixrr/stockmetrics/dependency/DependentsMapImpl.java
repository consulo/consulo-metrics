...
public class DependentsMapImpl implements DependentsMap
{
	...
	private final Map<PsiClass, Bag<PsiJavaPackage>> packageDependents = new HashMap<PsiClass, Bag<PsiJavaPackage>>(1024);
	private final Map<PsiJavaPackage, Bag<PsiJavaPackage>> packageToPackageDependents = new HashMap<PsiJavaPackage, Bag<PsiJavaPackage>>(1024);
	private final Map<PsiClass, Set<PsiClass>> transitiveDependents = new HashMap<PsiClass, Set<PsiClass>>(1024);
	private final Map<PsiJavaPackage, Set<PsiJavaPackage>> transitivePackageDependents = new HashMap<PsiJavaPackage, Set<PsiJavaPackage>>(1024);
	...
	@Override
	public Set<PsiJavaPackage> calculatePackageDependents(PsiClass aClass)
	{
		final Bag<PsiJavaPackage> existing = packageDependents.get(aClass);
		if(existing == null)
		{
			return Collections.emptySet();
		}
	...
	@Override
	public Set<PsiJavaPackage> calculatePackageToPackageDependents(PsiJavaPackage packageName)
	{
		final Bag<PsiJavaPackage> existing = packageToPackageDependents.get(packageName);
		if(existing == null)
		{
			return Collections.emptySet();
		}
	...
	@Override
	public int getStrengthForPackageDependent(PsiClass aClass, PsiJavaPackage dependentPackage)
	{
		final Bag<PsiJavaPackage> dependentsForClass = packageDependents.get(aClass);
	...
	@Override
	public Set<PsiJavaPackage> calculateTransitivePackageDependents(PsiJavaPackage packageName)
	{
		final Set<PsiJavaPackage> out = transitivePackageDependents.get(packageName);
		if(out != null)
		{
			return out;
		}
		...
		final Set<PsiJavaPackage> allDependents = new HashSet<PsiJavaPackage>();
		while(!pendingPackages.isEmpty())
		{
			final PsiJavaPackage dependentPackage = pendingPackages.get(0);
			...
			if(!allDependents.contains(dependentPackage))
			{
				...
				else
				{
					...
					final Set<PsiJavaPackage> indirectDependents = calculatePackageToPackageDependents(dependentPackage);
	...
	private class DependenciesVisitor extends JavaRecursiveElementVisitor
	{
		...
		private PsiJavaPackage currentPackage = null;

		private void addDependent(PsiClass aClass, PsiClass dependentClass, PsiJavaPackage dependentPackage)
		{
			...
			Bag<PsiJavaPackage> packageDependentsForClass = packageDependents.get(aClass);
			...
			Bag<PsiJavaPackage> packageDependentsForPackage = packageToPackageDependents.get(aPackage);
